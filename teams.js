$(document).ready(function(){
    var tDetails=JSON.parse(localStorage.getItem("tDetails"));
    $(".squad-img").attr("src",tDetails.teamIcon);
    $(".squad-name").html(tDetails.teamName);
    $(".player-count").html(tDetails.playerCount);
    $(".top-batsman").html(tDetails.topBatsman);
    $(".top-bowler").html(tDetails.topBowler);
    $(".matches-won").html(tDetails.winningStreak);
    var playerList=[{
        "Name":"Virat Kohli",
        "Role":"Batsman",
        "Team":"RCB",
    ...
    for(var i=0;i<playerList.length;i++){
        if(tDetails.teamName==playerList[i].Team){
    ...
    for(var j=0;j<teamPlayerList.length;j++){
        ...
        var status=$("<p>").addClass("card-text").html(teamPlayerList[j].pLayingStatus);
        var title=$("<h5>").addClass("card-title").html(teamPlayerList[j].Name);
        var text= $("<div>").addClass("card-body").append(title,price,status,btn);
        var icon=$("<img>").attr("src",teamPlayerList[j].photo).addClass("card-img-top");
        var card=$("<div>").addClass("card").css("width","28rem");
        card.append(icon,text);
        $(".squad-card-container").append(card);
    }
    $(".btn").click(function(){
        for(var i=0;i<teamPlayerList.length;i++){
            if($(this).siblings("h5").html()==teamPlayerList[i].Name){
                localStorage.setItem("pDetails",JSON.stringify(teamPlayerList[i]));
            }
        }
    });
    $('.search-input-box').on("keyup", function() {
        ...
        $(".card").each(function(){
            var squadfound="false";
            var tit=$(this).children(".card-body").children(".card-title").html().toUpperCase();
            if(tit.indexOf(value)>=0)
            squadfound="true";
            if(squadfound=="true")
            $(this).show();
            else
